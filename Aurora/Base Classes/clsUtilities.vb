...
#Region "Generate XML"
    Public Sub GenerateXML(ByVal strDocEntry As String)
        Try
            ...
            If oDoc.GetByKey(strDocEntry) Then
                oDoc_Lines = oDoc.Lines
                Dim strPath As String = strDocEntry.ToString() + "_" + System.DateTime.Now.Date.ToString("yyyyMMdd") + ".xml"
                Dim Doc As New Xml.XmlDocument
                Dim dec As XmlDeclaration = Doc.CreateXmlDeclaration("1.0", Nothing, Nothing)
                Doc.AppendChild(dec)

                Dim Invoices As XmlElement = Doc.CreateElement("Invoices")

                Dim From As XmlElement = Doc.CreateElement("From")
                From.InnerText = oApplication.Company.CompanyName
                Invoices.AppendChild(From)

                Dim RegisteredAddress As XmlElement = Doc.CreateElement("RegisteredAddress")
                RegisteredAddress.InnerText = oApplication.Company.GetCompanyService().GetAdminInfo().Address
                Invoices.AppendChild(RegisteredAddress)

                Dim CorrespondenceAddress As XmlElement = Doc.CreateElement("CorrespondenceAddress")
                CorrespondenceAddress.InnerText = oApplication.Company.GetCompanyService().GetAdminInfo().Address
                Invoices.AppendChild(CorrespondenceAddress)

                Dim Invoice As XmlElement = Doc.CreateElement("Invoice")

                Dim docNo As XmlElement = Doc.CreateElement("InvoiceNumber")
                docNo.InnerText = oDoc.DocNum
                Invoice.AppendChild(docNo)

                Dim docDate As XmlElement = Doc.CreateElement("InvoiceDate")
                docDate.InnerText = oDoc.DocDate.ToString("dd/MM/yyyy")
                Invoice.AppendChild(docDate)

                Dim docDueDate As XmlElement = Doc.CreateElement("DueDate")
                docDueDate.InnerText = oDoc.DocDueDate.ToString("dd/MM/yyyy")
                Invoice.AppendChild(docDueDate)

                Dim docInvType As XmlElement = Doc.CreateElement("InvoiceType")
                docInvType.InnerText = oDoc.UserFields.Fields.Item("U_InvType").Value
                Invoice.AppendChild(docInvType)

                Dim docCustomer As XmlElement = Doc.CreateElement("Customer")
                docCustomer.InnerText = oDoc.CardCode
                Invoice.AppendChild(docCustomer)

                Dim docName As XmlElement = Doc.CreateElement("Name")
                docName.InnerText = oDoc.CardName
                Invoice.AppendChild(docName)

                Dim docRef As XmlElement = Doc.CreateElement("OrderReference")
                Dim oRes As SAPbobsCOM.Recordset
                oRes = oApplication.Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
                oRes.DoQuery("Select isnull(ImportEnt,'') from OINV where Docentry=" & oDoc.DocEntry)
                docRef.InnerText = oRes.Fields.Item(0).Value
                Invoice.AppendChild(docRef)

                Dim docBVTotal As XmlElement = Doc.CreateElement("TotalBeforeVAT")
                docBVTotal.InnerText = oDoc.DocTotal - oDoc.VatSum
                Invoice.AppendChild(docBVTotal)

                Dim docVAT As XmlElement = Doc.CreateElement("VATAmount")
                docVAT.InnerText = oDoc.VatSum
                Invoice.AppendChild(docVAT)

                Dim docTotal As XmlElement = Doc.CreateElement("GrandTotal")
                docTotal.InnerText = oDoc.DocTotal
                Invoice.AppendChild(docTotal)

                Dim docDiscount As XmlElement = Doc.CreateElement("Discount")
                docDiscount.InnerText = (oDoc.DiscountPercent * (oDoc.DocTotal - oDoc.VatSum)) / 100
                Invoice.AppendChild(docDiscount)

                Dim docCurrency As XmlElement = Doc.CreateElement("Currency")
                docCurrency.InnerText = oDoc.DocCurrency
                Invoice.AppendChild(docCurrency)

                Dim docRemarks As XmlElement = Doc.CreateElement("Remarks")
                docRemarks.InnerText = oDoc.Comments
                Invoice.AppendChild(docRemarks)

                Dim InvoiceLines As XmlElement = Doc.CreateElement("InvoiceLines")

                Dim x As Integer
                For x = 0 To oDoc_Lines.Count - 1
                    oDoc_Lines.SetCurrentLine(x)
                    Dim InvoiceLine As XmlNode = Doc.CreateElement("InvoiceLine")

                    Dim InvoiceNumber As XmlElement
                    InvoiceNumber = Doc.CreateElement("InvoiceNumber")
                    InvoiceNumber.InnerText = oDoc.DocNum
                    InvoiceLine.AppendChild(InvoiceNumber)

                    Dim LineNumber As XmlElement
                    LineNumber = Doc.CreateElement("LineNumber")
                    LineNumber.InnerText = (x + 1).ToString()
                    InvoiceLine.AppendChild(LineNumber)

                    Dim ItemName As XmlElement
                    ItemName = Doc.CreateElement("ItemName")
                    ItemName.InnerText = oDoc_Lines.ItemCode
                    InvoiceLine.AppendChild(ItemName)

                    Dim Description As XmlElement
                    Description = Doc.CreateElement("Description")
                    Description.InnerText = oDoc_Lines.ItemDescription
                    InvoiceLine.AppendChild(Description)

                    Dim AircraftRegistration As XmlElement
                    AircraftRegistration = Doc.CreateElement("AircraftRegistration")
                    AircraftRegistration.InnerText = oDoc.UserFields.Fields.Item("U_Aircraft_regn").Value
                    InvoiceLine.AppendChild(AircraftRegistration)

                    Dim AircraftType As XmlElement
                    AircraftType = Doc.CreateElement("AircraftType")
                    AircraftType.InnerText = oDoc.UserFields.Fields.Item("U_Aircraft_type").Value
                    InvoiceLine.AppendChild(AircraftType)

                    Dim FlightDate As XmlElement
                    FlightDate = Doc.CreateElement("FlightDate")
                    FlightDate.InnerText = oDoc.DocDate.ToString("dd/MM/yyyy")
                    InvoiceLine.AppendChild(FlightDate)

                    Dim FlightNumber As XmlElement
                    FlightNumber = Doc.CreateElement("FlightNumber")
                    FlightNumber.InnerText = oDoc.UserFields.Fields.Item("U_Flightnumber").Value
                    InvoiceLine.AppendChild(FlightNumber)

                    Dim AirportTimeArrival As XmlElement
                    AirportTimeArrival = Doc.CreateElement("AirportTimeArrival")
                    AirportTimeArrival.InnerText = oDoc.UserFields.Fields.Item("U_AirTA").Value
                    InvoiceLine.AppendChild(AirportTimeArrival)

                    Dim AirportTimeDeparture As XmlElement
                    AirportTimeDeparture = Doc.CreateElement("AirportTimeDeparture")
                    AirportTimeDeparture.InnerText = oDoc.UserFields.Fields.Item("U_AirTD").Value
                    InvoiceLine.AppendChild(AirportTimeDeparture)

                    Dim Country As XmlElement
                    Country = Doc.CreateElement("Country")
                    Country.InnerText = oDoc_Lines.UserFields.Fields.Item("U_Country").Value
                    InvoiceLine.AppendChild(Country)

                    Dim AirportICAO As XmlElement
                    AirportICAO = Doc.CreateElement("AirportICAO")
                    AirportICAO.InnerText = oDoc_Lines.UserFields.Fields.Item("U_airport").Value
                    InvoiceLine.AppendChild(AirportICAO)

                    Dim AirportIATA As XmlElement
                    AirportIATA = Doc.CreateElement("AirportIATA")
                    AirportIATA.InnerText = oDoc_Lines.UserFields.Fields.Item("U_IATA").Value
                    InvoiceLine.AppendChild(AirportIATA)

                    Dim Price As XmlElement
                    Price = Doc.CreateElement("Price")
                    Price.InnerText = oDoc_Lines.UnitPrice
                    InvoiceLine.AppendChild(Price)

                    Dim Quantity As XmlElement
                    Quantity = Doc.CreateElement("Quantity")
                    Quantity.InnerText = oDoc_Lines.Quantity
                    InvoiceLine.AppendChild(Quantity)

                    Dim Unit As XmlElement
                    Unit = Doc.CreateElement("Unit")
                    Unit.InnerText = oDoc_Lines.UnitsOfMeasurment
                    InvoiceLine.AppendChild(Unit)

                    Dim Amount As XmlElement
                    Amount = Doc.CreateElement("Amount")
                    Amount.InnerText = oDoc_Lines.LineTotal
